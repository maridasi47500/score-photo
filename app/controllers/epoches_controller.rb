class EpochesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @epoch.update(epoch_params)
        format.html { redirect_to @epoch, notice: "Epoch was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to epoches_path, notice: "Epoch was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_epoch
      @epoch = Epoch.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def epoch_params
      params.expect(epoch: [ :name ])
    end
end
